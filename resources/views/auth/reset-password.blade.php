...
<div class="relative z-10 w-full max-w-[400px]">
    <div class="bg-white rounded-lg shadow-2xl overflow-hidden border-t-[3px] border-indigo-600">
        ...
        <div class="p-6">
            <p class="text-center text-slate-500 mb-5">Anda hanya selangkah lagi dari kata sandi baru Anda, pulihkan kata sandi Anda sekarang.</p>
            ...
            <form method="POST" action="#" x-data="{ loading: false }" @submit="loading = true">
                ...
                <div class="space-y-4 mb-5">
                    <div class="relative" x-data="{ showPassword: false }">
                        <input id="resetPassword" name="password" :type="showPassword ? 'text' : 'password'" required
                        ...
                        <label for="resetPassword" class="absolute left-3 top-2 text-xs text-slate-500 transition-all peer-placeholder-shown:top-[14px] peer-placeholder-shown:text-sm peer-focus:top-2 peer-focus:text-xs peer-focus:text-indigo-600">Kata Sandi Baru</label>
                    ...
                    <div class="relative" x-data="{ showPassword: false }">
                        <input id="resetPasswordConfirm" name="password_confirmation" :type="showPassword ? 'text' : 'password'" required
                            class="peer w-full rounded-md border border-slate-300 px-3 pr-10 pb-2 pt-6 text-sm text-slate-900 focus:border-indigo-600 focus:outline-none focus:ring-1 focus:ring-indigo-600 transition-colors placeholder-transparent"
                            placeholder="Konfirmasi Kata Sandi" />
                        <label for="resetPasswordConfirm" class="absolute left-3 top-2 text-xs text-slate-500 transition-all peer-placeholder-shown:top-[14px] peer-placeholder-shown:text-sm peer-focus:top-2 peer-focus:text-xs peer-focus:text-indigo-600">Konfirmasi Kata Sandi</label>
                        <button type="button" @click="showPassword = !showPassword" class="absolute inset-y-0 right-0 flex items-center pr-3">
                            <svg x-show="!showPassword" class="w-5 h-5 text-slate-400 hover:text-slate-600 transition-colors" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                              <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                              <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                ...
                <div class="mb-5">
                    <button type="submit" :disabled="loading" class="w-full rounded bg-indigo-600 px-4 py-2 text-[15px] font-semibold text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-offset-2 transition-colors flex justify-center items-center shadow-sm">
                        ...
                        <span x-show="loading" style="display:none;">Mengubah...</span>
                    </button>
                </div>
            </form>

            <div class="mt-4 flex flex-col space-y-1 text-[15px] text-center">
                <a href="{{ route('login') }}" class="text-indigo-600 hover:text-indigo-800 transition-colors mt-2">Kembali ke halaman Masuk</a>
            </div>
